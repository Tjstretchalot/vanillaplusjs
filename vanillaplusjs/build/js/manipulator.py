...
class JSManipulator:
    """Describes something which is capable of manipulating javascript (JS)
    in a structured way.
    ...
    def start_mark(self, node: JSToken) -> bool:
        ...
        Args:
            node (JSToken): The node to potentially mark
    ...
    def continue_mark(self, node: JSToken) -> Optional[List[JSToken]]:
        ...
        Args:
            node (JSToken): The next marked node

        Returns:
            (list[JSToken], None): Either None, to continue the mark,

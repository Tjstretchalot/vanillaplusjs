...
class CSSManipulator:
    """Describes something which is capable of manipulating cascading stylesheets (CSS)
    in a structured way.
    ...
    def start_mark(self, node: CSSToken) -> bool:
        ...
        Args:
            node (CSSToken): The node to potentially mark
    ...
    def continue_mark(self, node: CSSToken) -> Optional[List[CSSToken]]:
        ...
        Args:
            node (CSSToken): The next marked node

        Returns:
            (list[CSSToken], None): Either None, to continue the mark,
                or a list of nodes to replace the marked nodes with.
        """
        return []
